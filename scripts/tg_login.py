...
prints contains a secret.
    ...
    railway run py -3.11 scripts/tg_login.py --name lookup_userbot  # to deploy
    railway run py -3.11 scripts/tg_login.py --name lookup_local    # local twin

The only session the bot still needs is the phone-lookup account's
(TELEGRAM_LOOKUP_SESSION); the roster read runs on the bot token.
...
deployed bot was also used by a local script. Keep `lookup_local` for your
machine and `lookup_userbot` for Railway; never point both at one file. Extra
...
def main() -> None:
    ...
    ap.add_argument("--name", default="lookup_local",
                    help="session file name under ~/.pti-tg (default: lookup_local)")
    args = ap.parse_args()
    ...
    if not api_id or not api_hash:
        raise SystemExit(
            ...
            "Either run this under `railway run --service <bot service>`, "
